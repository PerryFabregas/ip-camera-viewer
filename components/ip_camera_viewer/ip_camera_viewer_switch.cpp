...
bool IPCameraViewer::fetch_jpeg_frame_() {
  ...
  if (now - this->stream_connect_time_ > this->stream_reconnect_interval_) {
    ...
    this->disconnect_mjpeg_stream_();
    // Was delay(500) - fine for the original 180-600s interval, but this is
    // now also used as a short-interval "always latest" mechanism (a few
    // seconds), where 500ms blocked out of every few seconds adds up to
    // real, noticeable UI lag. 50ms still gives the WiFi stack a brief beat
    // to settle without eating a large chunk of every cycle.
    delay(50);
