#include "edge264_internal.h"
	...
	#define broadcasthi32(a) (i16x8)_mm_shuffle_epi32(_mm_shufflehi_epi16(a, _MM_SHUFFLE(2, 2, 0, 0)), _MM_SHUFFLE(3, 3, 2, 2))
	...
	static always_inline i16x8 scale32(i32x4 c0, i32x4 c1, u16x8 ls, i32x4 off, i64x2 sh) {
	...
	#define shrrs32(a, i, off) (i32x4)vrshrq_n_s32(a, i)
		...
		static always_inline i16x8 scale32(i32x4 c0, i32x4 c1, u16x8 ls, int _, int sh) {
			i32x4 a = vmull_n_s16(vget_low_s16(ls), sh);
			i32x4 b = vmull_high_n_s16(ls, sh);
			return vrshrn_high_n_s32(vrshrn_n_s32(a * c0, 6), b * c1, 6);
		}
	#else
		#define addhiu8s16(a, b) (i16x8)vaddw_u8(b, vget_high_u8(a))
		#define mulhiu8(a, b) (u16x8)vmull_u8(vget_high_u8(a), vget_high_u8(b))
		...
		static always_inline i16x8 scale32(i32x4 c0, i32x4 c1, u16x8 ls, int _, int sh) {
			i32x4 a = vmull_n_s16(vget_low_s16(ls), sh);
			i32x4 b = vmull_n_s16(vget_high_s16(ls), sh);
	...
	#define addhiu8s16(a, b) ((i16x8)cvthi8u16(a) + (i16x8)(b))
	#define broadcastlo32(a) __builtin_shufflevector((i16x8)(a), (i16x8){}, 0, 0, 0, 0, 2, 2, 2, 2)
	#define broadcasthi32(a) __builtin_shufflevector((i16x8)(a), (i16x8){}, 4, 4, 4, 4, 6, 6, 6, 6)
	#define mullou8(a, b) (cvtlo8u16(a) * cvtlo8u16(b))
	#define mulhiu8(a, b) (cvthi8u16(a) * cvthi8u16(b))
	#define shlrrs32(a, l, r, off) ((((i32x4)(a) << (l)) + (off)) >> (r))
	#define shrrs32(a, i, off) (((i32x4)(a) + (off)) >> (i))
	#define shrps32(a, b, i) packs32((i32x4)(a) >> (i), (i32x4)(b) >> (i))
	static inline i16x8 scale32(i32x4 c0, i32x4 c1, u16x8 ls, i32x4 off, i32x4 sh) {
		// convertvector is sufficient here but packs should be more common in ISAs
		return packs32(((i32x4)cvtlo16u32(ls) * c0 + off) >> sh, ((i32x4)cvthi16u32(ls) * c1 + off) >> sh);
	}
#endif
...
static void add_idct8x8(Edge264Context *ctx, int iYCbCr, uint8_t *dst0)
{
	...
	if (ctx->t.samples_clip[iYCbCr][0] == 255) {
		...
		if (__builtin_expect(div < 6, 1)) {
			...
			#elif SIMD == CLANG
				i32x4 off = set32(1 << (5 - div));
				i32x4 sh = set32(6 - div);
			#endif
			d0 = scale32(c[0], c[1], LS0, off, sh);
			d1 = scale32(c[2], c[3], LS1, off, sh);
			d2 = scale32(c[4], c[5], LS2, off, sh);
			d3 = scale32(c[6], c[7], LS3, off, sh);
			d4 = scale32(c[8], c[9], LS4, off, sh);
			d5 = scale32(c[10], c[11], LS5, off, sh);
			d6 = scale32(c[12], c[13], LS6, off, sh);
...
static void transform_dc4x4(Edge264Context *ctx, int iYCbCr)
{
	...
	// ... or prepare for storage in place
		...
		i32x4 r2 = (dc2 + s32) >> 6;
		i32x4 r3 = (dc3 + s32) >> 6;
		i16x8 lo0 = broadcastlo32(r0);
		i16x8 hi0 = broadcasthi32(r0);
		i16x8 lo1 = broadcastlo32(r1);
		i16x8 hi1 = broadcasthi32(r1);
		i16x8 lo2 = broadcastlo32(r2);
		i16x8 hi2 = broadcasthi32(r2);
		i16x8 lo3 = broadcastlo32(r3);
		i16x8 hi3 = broadcasthi32(r3);
...
static void transform_dc2x2(Edge264Context *ctx)
{
	...
	// ... or prepare for storage in place
		...
		i16x8 lob = broadcastlo32(rb);
		i16x8 hib = broadcasthi32(rb);
		i16x8 lor = broadcastlo32(rr);
		i16x8 hir = broadcasthi32(rr);
